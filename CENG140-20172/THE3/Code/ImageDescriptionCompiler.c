...
Boolean saveImage ( Image image , char * fileName )
	{
		FILE *f;
		unsigned i,maks;
		maks = (unsigned)image.height*(unsigned)image.width;
		f = fopen(fileName,"w");
		fprintf(f,"P3\n# %s\n%d %d\n255\n",fileName,image.width,image.height);
		for(i = 0; i<maks; ++i)
			{
				fprintf(f,"%d %d %d ",image.pixels[i].red,image.pixels[i].green,image.pixels[i].blue);
				if(!((i+1)%5))
					fprintf(f,"\n");
			}
		fclose(f);
		/*fprintf(stderr,"%s is saved successfully...\n",fileName);*/
		return TRUE;
	}
...
(
  ...
  Color        color
)
	{
		...
		double centerAngle, totalAngle;
		Point *edges;
		totalAngle = (endAngleDegrees - startAngleDegrees)*PI/180.0;
		centerAngle = totalAngle/numberOfSegments;
		edges = (Point*)malloc(sizeof(Point)*(numberOfSegments+1));
		for(i = 0; i<=numberOfSegments; i++)
			{
				unsigned short int distX,distY;
				distX = (unsigned short int)(radius*cos(-PI/2 + startAngleDegrees*PI/180.0 + i*centerAngle));
				distY = (unsigned short int)(radius*sin(-PI/2 + startAngleDegrees*PI/180.0 + i*centerAngle));
				edges[i]=makePoint(center.row+distY,center.col+distX);
			}
		for(i = 0; i<numberOfSegments; i++)
			{
				drawLine(image,edges[i],edges[i+1],color);
			}
		return TRUE;
	}
...
Boolean drawRectangle ( Image image , Point topLeft , unsigned short int width , unsigned short int height , Color color )
	{
		Point edge1,edge2,edge3,edge4;/*from top left edge in ccw direction*/
		edge1 = topLeft;
		edge2 = makePoint(edge1.row+height,edge1.col);
		edge3 = makePoint(edge2.row,edge2.col+width);
		edge4 = makePoint(edge1.row,edge3.col);
		drawLine(image,edge1,edge2,color);
		drawLine(image,edge2,edge3,color);
		drawLine(image,edge3,edge4,color);
		drawLine(image,edge4,edge1,color);
		return TRUE;
	}
...
Boolean drawRegularConvexPolygon ( Image image , Point center , unsigned short int radius , unsigned short int numberOfSides , Color color )
	{
		...
		double centerAngle;
		Point *edges;
		centerAngle = 2*PI/numberOfSides;
		edges = (Point*)malloc(sizeof(Point)*numberOfSides);
		for(i = 0; i<numberOfSides; i++)
			{
				unsigned short int distX,distY;
				distX = (unsigned short int)(radius*cos(-PI/2 + i*centerAngle));
				distY = (unsigned short int)(radius*sin(-PI/2 + i*centerAngle));
				edges[i]=makePoint(center.row+distY,center.col+distX);
			}
		...
		drawLine(image,edges[numberOfSides-1],edges[0],color);
		return TRUE;
	}
		...
		for(i = 12; i>=0; --i)
			{
				for(j = 7; j>=0; --j)
					{
						if(characterRasters[c-FIRST_CHARACTER][i] & pows[j])
							setPixel(image,makePoint(y,x),color);
						/*fprintf(stderr,"%d",characterRasters[c-FIRST_CHARACTER][i] & pows[j] ? 1 : 0);*/
						x++;
					}
				x = topLeft.col;
				y++;
